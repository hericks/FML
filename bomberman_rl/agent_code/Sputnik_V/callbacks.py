...
def state_to_features(game_state: dict) -> np.array:
    ...
    # crate features
    crate_features = get_first_step_to_nearest_object_features(free_tiles, pos, get_crates_list(field), 1)
    is_next_to_crate = np.array([len(get_crates_list(field)) == 2], dtype=np.int32)

    # coin features
    nearest_coin_path = get_nearest_object_path(free_tiles, pos, coins, 2)
    if len(nearest_coin_path) <= 10:
        coin_features = get_first_step_to_nearest_object_features(free_tiles, pos, coins, 2)
    else:
        coin_features = crate_features

    # bomb and escape death features
